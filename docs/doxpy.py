		...
		for transition in self.transitions:
			...
			if from_state == self.current_state:
				...
				if match:
					...
					if options.debug:
						print("# FSM: executing (%s -> %s) for line '%s'" % (from_state, to_state, input))
					callback(match)
					return
...
def outputDebugCallbackIfNeeded(inFunction):
	...
	def __wrappedFunction(*args, **kwargs):
		...
		if options.debug:
			print("# CALLBACK: %s" % inFunction.__name__)
...
class Doxypy(object):
	...
	def __closeComment(self):
		...
		if options.autobrief:
			while len(self.comment) > 0 and self.comment[0].strip() == '':
				del self.comment[0]
	...
	def appendCommentLine(self, match):
		...
		or (from_state == "FILEHEAD" and to_state == "FILEHEAD"):
			...
			self.comment.append(line[line.find(activeCommentDelim)+len(activeCommentDelim):line.rfind(activeCommentDelim)].rstrip())
		...
		# multiline end
			...
			activeCommentDelim = match.group(1)
			line = self.fsm.current_input
			self.comment.append(line[0:line.rfind(activeCommentDelim)].rstrip())
			if (to_state == "DEFCLASS_BODY"):
				self.__closeComment()
				self.defclass = []
